...
@property (nonatomic, assign) NSInteger autoscrollCounter;
@end
...
//        if (self.window)
        {
            _autoscrollCounter--;
            if (_autoscrollCounter <= 0 && _pagingEnabled) {
                if (!self.numberOfPages) return;
                [self scrollToPage:((self.currentPage+1)%self.numberOfPages) duration:0.5];
                _autoscrollCounter = _autoscroll;
